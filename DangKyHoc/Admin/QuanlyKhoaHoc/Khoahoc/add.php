<?php 
    ...
    $MKH= $Tenkhoahoc= $NgayDangKy=$HanDangKy=$Trangthai='';
    if (!empty($_POST)) {
        if (isset($_POST['MKH'])) {
            $MKH = $_POST['MKH'];
        }
        ...
        if (isset($_POST['NgayDangKy'])) {
            $NgayDangKy = $_POST['NgayDangKy'];
        }
        if (isset($_POST['HanDangKy'])) {
        ...
        if (!empty($Tenkhoahoc)) {
            //Luu vao database
            if ($MKH == '') {
                $sql1 = 'insert into Khoahoc(Tenkhoahoc,NgayDangKy,HanDangKy,Trangthai) 
                values ("'.$Tenkhoahoc.'","'.$NgayDangKy.'","'.$HanDangKy.'","'.$Trangthai.'")';
            } 
            else {
                $sql1 = 'update Khoahoc set Tenkhoahoc ="'.$Tenkhoahoc.'",NgayDangKy ="'.$NgayDangKy.'",HanDangKy ="'.$HanDangKy.'",Trangthai ="'.$Trangthai.'" where MKH = '.$MKH;
    ...
    if (isset($_GET['id'])) {
        ...
        $sql      = 'select * from Khoahoc where MKH= '.$MKH;
        $chitiet = executeSingleResult($sql);
        if ($chitiet != null) {
            $MKH          = $chitiet['MKH'];
            $Tenkhoahoc   = $chitiet['Tenkhoahoc'];
            $NgayDangKy   = $chitiet['NgayDangKy'];
            $HanDangKy    = $chitiet['HanDangKy'];
            $Trangthai    = $chitiet['Trangthai'];
            $NgayDangKy1  = str_replace(" ","T",$NgayDangKy); 
            $HanDangKy1   = str_replace(" ","T",$HanDangKy); 
        }
    }
?>
<!DOCTYPE html>
                            ...
                            <ul class="navbar-nav nav-pills" id="menu">
                                ...
                                <li class="nav-item">
                                    ...
                                    <ul class="collapse  nav flex-column" id="submenu1" data-bs-parent="#menu">
                                        <li class="w-100">
                                            <a href="index.php" class="nav-link px-2" data-bs-toggle="tooltip" data-bs-placement="right" title="Khoá học"><i class="bi bi-card-checklist icon-nav" style="padding-right:10px"></i><span class="ms-1">Khoá học</span></a>
                                        </li>
                                        <li>
                                            <a href="../Chitietkhoahoc/index.php" class="nav-link px-2" data-bs-toggle="tooltip" data-bs-placement="right" title="Chi tiết khoá học"><i class="bi bi-card-checklist icon-nav" style="padding-right:10px"></i><span class="ms-1">Chi tiết khoá học</span></a>
                                        </li>
                                        <li>
                        ...
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <?php if(isset($_GET['id'])) {?>
                                    <h2 class="text-center">Sửa Khoá học </h2>
                                <?php }else {?>
                                    <h2 class="text-center">Thêm Khoá học </h2>
                                <?php } ?>
                            </div>
                            <div class="panel-body">
                                <form method="post">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="MKH"  hidden="true"  value="<?=$MKH?>">
                                        <input required="true" type="text" class="form-control" id="Tenkhoahoc" name="Tenkhoahoc" value="<?=$Tenkhoahoc?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="name">Ngày đăng ký:</label>
                                        <input required="true" type="datetime-local" class="form-control" id="NgayDangKy" name="NgayDangKy" value="<?=$NgayDangKy1?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="name">Hạn đăng ký:</label>
                                        <input required="true" type="datetime-local" class="form-control" id="HanDangKy" name="HanDangKy" value="<?=$HanDangKy1?>">
                                    </div>
                                    <div class="form-group">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="Trangthai" id="Trangthai">
                                            ...
                                            <?php echo'<option selected value="'.$Trangthai.'">'?><?php echo("$Trangthai")?></option>;
                                            <option >Đóng</option>;
                                            <option >Mở</option>;
                                        </select>
                                    </div>
